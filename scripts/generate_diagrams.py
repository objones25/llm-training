...
def _arrow(ax, y_from, y_to, x=X_C):
    ax.annotate("", xy=(x, y_to), xytext=(x, y_from), arrowprops=ARROW_KW, zorder=2)
...
def make_arch(out_path: Path) -> None:
    ...
    b_em = d(
        p_em,
        "embed",
        ["Token Embedding", "vocab_size = 32 768   \u2192   d_model = 2 048"],
        True,
    )
    ...
    b_att = d(
        p_att,
        "attn",
        ["CausalSelfAttention", "32 heads   \u00b7   head_dim = 64"],
        True,
    )
    ...
    b_ff = d(
        p_ff, "ff", ["FeedForward", "2 048 \u2192 8 192 \u2192 2 048   \u00b7   GELU"]
    )
    ...
    b_lmh = d(
        p_lmh,
        "head",
        ["LM Head", "2 048 \u2192 32 768", "weight-tied to token embedding"],
        True,
    )
...
def make_attention(out_path: Path) -> None:
    ...
    b = d(
        p_qkv,
        "proj",
        ["QKV Projection", "Linear 2048 \u2192 6144   \u00b7   no bias"],
        True,
    )
    _arrow(ax, b, p_split[0])
    b = d(p_split, "proj", ["Split Q, K, V", "each   [B, T, 2048]"])
    _arrow(ax, b, p_reshape[0])
    b = d(p_reshape, "proj", ["Reshape + Transpose", "[B, 32, T, 64]"])
    ...
    b = d(
        ...
        [
            ...
            "softmax(QK\u1d40 / \u221a64) \u00b7 V",
            "FlashAttention-2 on CUDA",
        ],
        True,
    )
    _arrow(ax, b, p_reshp2[0])
    b = d(p_reshp2, "proj", ["Transpose + Reshape", "[B, T, 2048]"])
    _arrow(ax, b, p_outprj[0])
    b = d(
        p_outprj,
        "head",
        ["out_proj", "Linear 2048 \u2192 2048   \u00b7   no bias"],
        True,
    )
