...
class TrainConfig:
    ...
    dataset_config: str = (
        "sample-10BT"  # subset / config name passed as second arg to load_dataset
    )
    ...
    use_amp: bool = (
        False  # Automatic mixed precision — CUDA only; MPS does not support GradScaler
    )
    ...
    early_stopping_patience: int = (
        0  # val evals without improvement before stopping; 0 disables
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.ln_lr_mult <= 0:
            raise ValueError(f"ln_lr_mult must be positive, got {self.ln_lr_mult}")
